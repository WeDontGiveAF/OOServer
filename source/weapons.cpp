...
bool Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
	...
	if(g_config.getBool(ConfigManager::REMOVE_AMMUNITION)){
...
bool WeaponWand::configureEvent(xmlNodePtr p)
{
	...
	if(readXMLString(p, "type", strValue))
	{
		...
		if(tmpStrValue == "poison")
			...
			params.combatType = COMBAT_PHYSICALDAMAGE;
		else if(tmpStrValue == "undefined")
			params.combatType = COMBAT_UNDEFINEDDAMAGE;
		else if(tmpStrValue == "life drain" || tmpStrValue == "lifedrain")
			params.combatType = COMBAT_LIFEDRAIN;
		else if(tmpStrValue == "mana drain" || tmpStrValue == "manadrain")
			params.combatType = COMBAT_MANADRAIN;
		else
