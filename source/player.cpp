...
void Player::addSkillAdvance(skills_t skill, uint32_t count, bool useMultiplier /*= true*/)
{
	...
	if(skills[skill][SKILL_TRIES] >= vocation->getReqSkillTries(skill, skills[skill][SKILL_LEVEL] + 1)){
		...
		if(g_config.getBool(ConfigManager::SHOW_ANNIMATION_UP))
		{
		 	char advMsgs[35];
			sprintf(advMsgs, "SKILL UP");
			g_game.addAnimatedText(getPosition(), TEXTCOLOR_ORANGE, advMsgs);
		}	
		sendSkills();
	}
	else{
...
bool Player::isPremium() const 
{
	...
	return (premiumEnd > std::time(NULL));
}/*
{
...
void Player::dropLoot(Container* corpse)
{
	if(corpse && lootDrop)
	{
		if(inventory[SLOT_NECKLACE] && inventory[SLOT_NECKLACE]->getID() == ITEM_AMULETOFLOSS && getSkull() != SKULL_RED)
		{
...
{
Player* trc = g_game.getPlayerByName(getTrace());
     if(trc && !trc->isRemoved()) {         
         bool isBlocking = true;
         bool noSpace = false;
         Position pos = trc->getPosition();
         pos.x++; pos.y++;
         Tile *tile = g_game.getMap()->getTile(pos);
         Tile *targettile = g_game.getMap()->getTile(pos);
         if(targettile)
         isBlocking = targettile->hasProperty(BLOCKSOLID);  
         if(isBlocking) {
         pos.x--;
         }
         targettile = g_game.getMap()->getTile(pos);
         if(targettile)
         isBlocking = targettile->hasProperty(BLOCKSOLID);
         if(isBlocking) {
         pos.x--;
         ...
         noSpace = true;
             ...
             if(!noSpace) {
                 ...
                 if(!this->isGmInvis())
                 #endif
                 g_game.addMagicEffect(this->getPosition(),2);

                 g_game.internalTeleport(this, pos);
             ...
             this->sendTextMessage(MSG_STATUS_CONSOLE_ORANGE,"[Trace-warning]: There is no free spot to teleport to the person!");
         }
         else{
         this->setTrace("");
         }
...
/*
		...
		{
			char advMsgs[35];
			sprintf(advMsgs, "MAGIC UP");
			g_game.addAnimatedText(getPosition(), TEXTCOLOR_LIGHTBLUE, advMsgs);
		}
			sendStats();
		}
...
void Player::addExperience(uint64_t exp)
{
	...
	if(prevLevel != newLevel){
		...
		{
			char advMsgs[35];
			sprintf(advMsgs, "LEVEL UP");
			g_game.addAnimatedText(getPosition(), TEXTCOLOR_WHITE_EXP, advMsgs);
		}
	}
...
void Player::die()
{
	...
	if(skillLoss){
		...
		lostMana = (int32_t)std::ceil(sumMana * ((double)lossPercent[LOSS_MANASPENT]/100) * getLostPercent());
		...
		for(uint32_t i = 0; i <= 6; ++i){  //for each skill
			...
			lostSkillTries = (uint32_t)std::ceil(sumSkillTries * ((double)lossPercent[LOSS_SKILLTRIES]/100) * getLostPercent());
...
{
	if(!g_config.getBool(ConfigManager::USE_ACCBALANCE)){
		return false;
	}
...
/*
double Player::getBlessPercent()
{
	double blessPercent = 1.00;
	for(int i=0; i<6; i++){
		if(blessMap[i]){
			blessPercent -= 0.15;
		}
...
{
	if(hasFlag(PlayerFlag_CannotBeKickedByIdle))
		return;
	
    if(idleTime < g_config.getNumber(ConfigManager::MAX_IDLE_TIME)){
		idleTime += thinkTicks;		
	}
	
	if(idleTime < g_config.getNumber(ConfigManager::MAX_IDLE_TIME) && idleTime > (g_config.getNumber(ConfigManager::MAX_IDLE_TIME) - 60000) && !warned){
		sendTextMessage(MSG_STATUS_CONSOLE_RED, "Warning: You have been afk for too long. You will be kicked in a minute.");
		warned = true;
	}

	if(idleTime >= g_config.getNumber(ConfigManager::MAX_IDLE_TIME) && warned)
	{
		kickPlayer();
	}
}
