	...
	const Position& movingCreatureOrigPos, const Position& toPos)
{
	...
	if(!Position::areInRange<1,1,0>(movingCreatureOrigPos, player->getPosition()) && !player->hasFlag(PlayerFlag_CanMoveFromFar))
	{
...
bool Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, int index)
{
	...
	if(index == 0)
	{
		...
		if(player->hasFlag(PlayerFlag_CanSeeItemDetails))
		{
...
bool Game::playerLookAt(uint32_t playerId, const Position& pos, uint16_t spriteId, uint8_t stackPos)
{
	...
	if(player->hasFlag(PlayerFlag_CanSeeCreatureDetails))
	{
		if(Creature* creature = thing->getCreature())
		{
		...
		if(player->hasFlag(PlayerFlag_CanSeePosition))
...
bool Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
	...
	if(g_config.getBool(ConfigManager::CHANGE_OUTFIT))
    {
        player->sendCancel("You can not change your outfit.");
		return false;
	}
...
bool Game::playerReportRuleViolation(Player* player, const std::string& text)
{
	...
	if(g_config.getNumber(ConfigManager::ALLOW_CLONES) != 0)
	{
	...
	ChatChannel* channel = g_chat.getChannelById(CHANNEL_RULE_REP); //Rule Violations channel
	if(channel){
...
bool Game::bugReport(uint32_t playerId, std::string report)
{
	...
	DBQuery query;
	query << "INSERT INTO `server_reports` (`id`, `player_id`, `posx`, `posy`, `posz`, `timestamp`, `report`) VALUES (NULL, ";
	query << player->getGUID() << ", ";
	Position pos = player->getPosition();
	query << pos.x << ", " << pos.y << ", " << pos.z << ", ";
	query << time(NULL) << ", " << db->escapeString(report.c_str()) << ");";
...
void Game::autoSave()
{
	Dispatcher::getDispatcher().addTask(
		createTask(boost::bind(&Game::saveServer, this)));
...
void Game::prepareGlobalSave()
{
	if(!globalSaveMessage[0])
	{
		...
		anonymousBroadcastMessage(MSG_STATUS_WARNING, "Server is saving game in 5 minutes. Please logout.");
	...
	else if(!globalSaveMessage[1])
	{
		...
		anonymousBroadcastMessage(MSG_STATUS_WARNING, "Server is saving game in 3 minutes. Please logout.");
		Scheduler::getScheduler().addEvent(createSchedulerTask(120000, boost::bind(&Game::prepareGlobalSave, this)));
	}
	else if(!globalSaveMessage[2])
	{
		globalSaveMessage[2] = true;
		anonymousBroadcastMessage(MSG_STATUS_WARNING, "Server is saving game in 1 minutes. Please logout.");
...
void Game::loadMotd()
{
	...
	DBQuery query;
	query << "SELECT `id`, `text` FROM `server_motd` ORDER BY `id` DESC LIMIT 1";
