...
ChatChannel* Chat::getChannel(Player* player, uint16_t channelId)
{
	...
	if(nit != m_normalChannels.end())
	{
		if(channelId == CHANNEL_RULE_REP && !player->hasFlag(PlayerFlag_CanAnswerRuleViolations)) //Rule violations channel
			return NULL;
		else if(channelId == CHANNEL_TRADE && !player->hasFlag(PlayerFlag_GamemasterPrivileges) && player->getVocationId() == 0)
			return NULL;
		else if(channelId == CHANNEL_TRADE_ROOK && !player->hasFlag(PlayerFlag_GamemasterPrivileges) && player->getVocationId() != 0)
			return NULL;
