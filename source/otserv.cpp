...
{
    if(!chdir("/root/Downloads/server"))
        std::cout << "warning: failed to change dir" << std::endl;
	...
	//time(&start_time); TODO: what was start_time for?
#endif
	...
	std::cout << ":: Modified and fixed by: Tryller, Shenlong, Shynzo " << std::endl;
	std::cout << ":: ==============================================" << std::endl;
	std::cout << "::" << std::endl;
...
#if !defined(WIN32) && defined(__PRODUCTION__) && !defined(__ROOT_PERMISSION__)
...
/*
	...
	std::cout << ">> Checking SQL Connection... ";
	#if defined __USE_MYSQL__ && defined __USE_SQLITE__
	std::string sqlType = asLowerCaseString(g_config.getString(ConfigManager::SQL_TYPE));
	if(sqlType == "mysql" || sqlType == "MYSQL" || sqlType == "MySQL")
	{
		g_config.setNumber(ConfigManager::SQLTYPE, SQL_TYPE_MYSQL);
		std::cout << "MySQL." << std::endl;
		Database* db = Database::instance();
		if(db == NULL || !db->isConnected())
			ErrorMessage("Failed to connect to database, read misc/doc/HELP_MYSQL for information or try SqLite which doesn't require any connection.");
	}
	#elif defined __USE_MYSQL__
	std::cout << "MySQL." << std::endl;
	Database* db = Database::instance();
		if(db == NULL || !db->isConnected())
		ErrorMessage("Failed to connect to database, read doc/MYSQL_HELP for information or try SqLite which doesn't require any connection.");
	...
	fclose(sqliteFile);
	#else
	ErrorMessage("Unknown sqlType... terminating!");
	#endif
	...
	filename.str("");
	filename << g_config.getString(ConfigManager::DATA_DIRECTORY) << "XML/stages.xml";
