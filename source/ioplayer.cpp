...
bool IOPlayer::loadPlayer(Player* player, const std::string& name, bool preload /*= false*/)
{
	...
	//load blessings
	query.str("");
	query << "SELECT * FROM player_blessings WHERE player_id='" << player->getGUID() << "'";
	if((result = db->storeQuery(query.str()))){
		do{
			player->addBlessing(result->getDataInt("blessing"));
	}while(result->next());
        db->freeResult(result);  
    }
...
bool IOPlayer::savePlayer(Player* player)
{
	...
	if(!player->VIPList.empty()){
   	...
   	//Save Blessings
	query.str("");
	query << "DELETE FROM `player_blessings` WHERE player_id='" << player->getGUID() << "'";
	...
	query.str("");
	stmt.setQuery("INSERT INTO `player_blessings` (`player_id`, `blessing`) VALUES ");
	for(int i=0; i<6; i++){
		if(player->getBlessing(i)){
			query << "" << player->getGUID() << ", " << i << "";
		    if(!stmt.addRow(query)){
			     return false;
		    }
            query.str("");
